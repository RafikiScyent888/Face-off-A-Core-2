...
link only works in another tab on the host computer. To let 33 students join
from their own devices you need one free Firebase Realtime Database. It is free
forever at this scale (Firebase's free "Spark" plan allows 100 simultaneous
connections; you'll use about 9).
...
3. Name it something like `faceoff-core2`. Click **Continue**.
...
5. You'll land on a screen showing a URL like
   `https://faceoff-core2-default-rtdb.firebaseio.com/` — that's your `databaseURL`.
...
3. App nickname: `faceoff`. **Do not** check "Firebase Hosting". Click **Register app**.
...
const firebaseConfig = {
  ...
  authDomain: "faceoff-core2.firebaseapp.com",
  databaseURL: "https://faceoff-core2-default-rtdb.firebaseio.com",
  projectId: "faceoff-core2",
  storageBucket: "faceoff-core2.appspot.com",
...
- **Run it single-screen.** Set `enabled: false` and play off the projector
  alone: press keys **1–7** to buzz for a team. Give each team a physical
...
transfer. One class period of this game moves a few megabytes. You would need
to run it continuously for years to leave the free tier.
