...
function Host(forcedCode) {
  ...
  function makeTeams(n) {
    ...
    for (var i = 0; i < n; i++) {
      var t = old[i] || {
        ...
        members: [], score: 0, captain: null, locked: false, cls: 'A'
      };
      if (!t.cls) t.cls = 'A';
      S.teams.push(t);
    }
  ...
  function endRound() {
    ...
    Object.keys(need).forEach(function (c) {
      var pool = (S.settings.classMode ? aliveInClass(c) : aliveTeams())
  ...
  /* is anyone advancing level on points with anyone going out? */
  function tieAtLine(pool) {
    var cutScores = pool.filter(function (t) { return S.tour.cut.indexOf(t.id) >= 0; })
                        .map(function (t) { return t.score; });
    if (!cutScores.length) return false;
    var high = Math.max.apply(null, cutScores);
    return pool.some(function (t) {
      return S.tour.cut.indexOf(t.id) < 0 && t.score === high;
    });
  }
      ...
      '<div class="qbox" style="align-items:flex-start"><div style="width:min(1080px,100%)">' +
        ...
        '<div class="hint" style="text-align:center;margin-bottom:14px">' +
          (anyTie
            ...
            : 'Click any team to change who goes out') + '</div>' +
