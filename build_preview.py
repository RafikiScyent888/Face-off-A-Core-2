...
def bundle_app() -> str:
    ...
    for src in ["questions-core2.js", "firebase-config.js", "qr.js", "app.js"]:
        ...
        # index.html cache-busts with ?v=N — match the tag with or without it
        page = re.sub(rf'<script src="{re.escape(src)}(\?[^"]*)?"></script>',
                      lambda m: f"<script>\n/* ---- {src} ---- */\n{js}\n</script>",
                      page)
...
<header>
  <div class="mark">&#9876;</div>
  <div><h1>FACE-OFF &mdash; LIVE PREVIEW</h1><div class="sub">A+ CORE 2 &middot; 220-1202</div></div>
  <div class="steps">
