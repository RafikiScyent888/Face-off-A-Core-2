...
var MAX_TEAMS = 12, MAX_TEAM_SIZE = 8;
/* host-keyboard buzzers, in team order: 1-9, then 0, then - and = */
var BUZZ_KEYS = ['1','2','3','4','5','6','7','8','9','0','-','='];
...
/* boards played before only two teams remain, cutting 2 per round */
function boardsNeeded(teams) { return Math.max(1, Math.ceil((teams - 2) / 2)); }
...
function Host(forcedCode) {
  ...
  function startTournament() {
    ...
    S.tour = {
      stage: 0,
      totalBoards: boardsNeeded(S.teams.length),
      plan: plan,
      board: [],
  ...
  /* end of a board: propose the bottom teams for elimination */
  function endRound() {
    if (S.lightning) return;
    var alive = aliveTeams().slice().sort(function (a, b) { return a.score - b.score; });
    var n = cutSize(alive.length);
    if (n <= 0) { startLightning(); return; }
    S.tour.cut = alive.slice(0, n).map(function (t) { return t.id; });
    S.phase = 'cut';
    stopTimer(); sync();
  }
  ...
  function applyCut() {
    var need = cutSize(aliveTeams().length);
    if (S.tour.cut.length !== need) {
      flash('Pick exactly ' + need + ' team' + (need === 1 ? '' : 's') + ' to eliminate', 'bad');
      return;
    }
    ...
    Snd.wrong();
    if (S.tour.alive.length <= 2) { startLightning(); return; }
    S.tour.stage++;
    dealBoard();
  }
  ...
  function scoreStrip() {
    ...
    return '<div class="scores' + (list.length > 8 ? ' many' : '') + '">' + list.map(function (t) {
  ...
  function lightningView() {
    ...
    return '<div class="boardwrap">' +
      '<div class="lightwrap">' +
        ...
        '<div class="lightsub">Fastest buzz wins it · ' + S.settings.lightningSecs + ' seconds · 500 points a question</div>' +
  ...
  function boardView() {
    ...
    if (!b.length) {
      return '<div class="wrap"><div class="card"><h2>No questions loaded</h2>' +
        '<div class="hint">Add categories to <span class="mono">questions-core2.js</span>.</div></div></div>';
    }
  ...
  function cutView() {
    var need = cutSize(aliveTeams().length);
    var doomedSet = S.tour.cut;
    /* survivors first, then the teams on the chopping block — within each, high to low */
    var alive = aliveTeams().slice().sort(function (a, b) {
      var ad = doomedSet.indexOf(a.id) >= 0, bd = doomedSet.indexOf(b.id) >= 0;
      if (ad !== bd) return ad ? 1 : -1;
      return b.score - a.score;
    });
    /* a tie matters only at the cut LINE: someone advancing level with someone going out */
    var cutScores = doomedSet.map(function (id) { var x = team(id); return x ? x.score : 0; });
    var highestCut = cutScores.length ? Math.max.apply(null, cutScores) : null;
    var tied = highestCut !== null && alive.some(function (t) {
      return doomedSet.indexOf(t.id) < 0 && t.score === highestCut;
    });
    return '<div class="overlay">' +
      '<div class="ovtop"><div class="ovcat">END OF ROUND ' + (S.tour.stage + 1) + '</div>' +
        '<div class="spacer"></div><div class="objtag">' + alive.length + ' in · ' + need + ' going out</div></div>' +
      '<div class="qbox"><div style="width:min(760px,100%)">' +
        '<h1 style="font-size:clamp(26px,4vw,44px);margin:0 0 4px;text-align:center">STANDINGS</h1>' +
        '<div class="hint" style="text-align:center;margin-bottom:18px">' +
          (tied
            ? '<b style="color:var(--royal-yellow)">TIED AT THE CUT LINE</b> — teams are level on points across the boundary. Click any team to choose who goes out.'
            : 'Click any team to change who goes out') +
        '</div>' +
        alive.map(function (t, i) {
          ...
          return '<div class="rankrow' + (doomed ? ' doomed' : '') + '" data-cut="' + t.id + '">' +
            ...
            '<span class="tag">' + (doomed ? 'ELIMINATED' : 'ADVANCES') + '</span>' +
          '</div>';
        }).join('') +
      '</div></div>' +
      '<div class="row" style="justify-content:center">' +
        '<button class="btn primary lg" data-act="applycut">' +
          (alive.length - S.tour.cut.length <= 2 ? 'Eliminate & start the Lightning Final →'
  ...
  function settingsModal() {
    var plan = lengthPlan(S.settings.lengthMinutes);
    var boards = boardsNeeded(S.settings.teamCount);
    return '<div class="modal"><div class="card">' +
      ...
      '<div class="notice" style="margin-top:13px">' +
        '<b>Room:</b> ' + (S.settings.teamCount * S.settings.teamSize) + ' students (' +
          S.settings.teamCount + ' teams × ' + S.settings.teamSize + '). A 40-student class fits 8 × 5.<br>' +
        '<b>Bracket:</b> ' + boards + ' board' + (boards === 1 ? '' : 's') + ' of ' + plan.cats + ' × ' + plan.rows +
          ', dropping the bottom 2 each round, then a ' + plan.lightning + '-question Lightning Final between the last two.' +
...
function Player(code, seat) {
  ...
  function playView() {
    ...
    if (iAmOut) {
      ...
      return header() + body0 + standingsBlock('Tournament standings');
    }
