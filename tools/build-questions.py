...
def build():
    ...
    # Lightning final: the tightest question/answer pairs in the bank, at most
    # two per sub-objective so the head-to-head still spans the whole exam.
    # The pool has to outlast a 15-question final plus sudden death.
    for cat in cats:
        taken = 0
        for c in cat["clues"]:
            if len(c["q"]) <= 175 and len(c["a"]) <= 46:
                lightning.append(c)
                taken += 1
                if taken == 2:
                    break
